...
class DateTime{
	 ...
	 *
	 * Example: "{ timeFormat: 'h:mm tt', controlType: 'select'}";
	 *
	 * @var string
	 */
	public $options = "";
	 ...
	 * Constructor
	 *
	 * @param string $type	The type of datetime input (Allowable values are: date, datetime, time)
     */
	...
	public function render($outputAs='html'){
		...
		// Create <INPUT> tag
		$output = "<input id='".$this->prefix.$id."' name='".$id."' type='text' value='".(htmlentities($contents, ENT_QUOTES,'UTF-8'))."' ".$attributes.">";

		// Add some jquery
		$options = "{}";
		if(isset($this->client->input[$id]->options)){
			$options = $this->client->input[$id]->options;
		}

		// Date or DateTime or Time?
		if($this->type == "datetime"){
			$js = "<script> $(function(){
					    $('#".$this->prefix.$id."').datetimepicker(".$options.");
				   }); </script>";
		}
		elseif($this->type == "time"){
			$js = "<script> $(function(){
					    $('#".$this->prefix.$id."').timepicker(".$options.");
				   }); </script>";
		}
		else{
			$js = "<script> $(function(){
					    $('#".$this->prefix.$id."').datepicker(".$options.");
				   }); </script>";
		}
		...
		elseif($outputAs == 'jquery'){
			return '$("#'.$this->prefix.$id.'").val("'.(htmlentities($contents, ENT_QUOTES,'UTF-8')).'");';
		}
	}
